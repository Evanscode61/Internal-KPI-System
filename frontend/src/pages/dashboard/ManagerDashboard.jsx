...
export default function ManagerDashboard({ data, alerts, onAlertResolved }) {
  ...
  return (
    <>
      ...
      <HealthScore
        ...
        title        = {`${data?.department || 'Department'} Health Score`}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-[#1a1d27] border border-white/10 rounded-2xl p-5 flex flex-col gap-4">
          <h2 className="text-white font-semibold text-sm">Teams — {data?.department}</h2>
          {(data?.teams || []).length === 0 && (
            <p className="text-slate-600 text-sm italic">No team data</p>
          )}
          {(data?.teams || []).map(team => (
            <div key={team.team_uuid} className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="text-white text-sm font-medium">{team.name}</span>
                <div className="flex items-center gap-2">
                  {team.weighted_score !== null && (
                    <span className="text-slate-400 text-xs">{team.weighted_score}%</span>
                  )}
              ...
              <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: team.weighted_score ? `${Math.min(team.weighted_score, 100)}%` : '0%' }}
                />
              </div>
            </div>
          ))}
