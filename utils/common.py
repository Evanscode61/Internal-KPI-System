...
def parse_request_body(request) -> dict:
    ...
    try:
        ...
        raise ValueError("Invalid JSON — ensure Content-Type: application/json and body is valid JSON")
...
def clean_data(
    ...
    if required_fields:
        missing = [f for f in required_fields if cleaned.get(f) is None]
        if missing:
...
def get_clean_request_data(
    ...
    """
      ...
      Extracts the request body, ensures required fields exist, and filters
      the data to only allowed fields if provided.
      Returns:dict: Cleaned request data.
      """
